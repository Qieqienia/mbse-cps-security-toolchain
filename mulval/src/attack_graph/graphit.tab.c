...
#line 1 "graphit.y"
    ...
    extern graph_data graph_data_global;
...
#line 78 "graphit.y"
                     {
                        graph_data_global.all_predicates.add_predicate( yyvsp[-5], atoi(yyvsp[-3]), 
                                      primitive);
                     }
...
#line 110 "graphit.y"
		...
		strlen(facts_str) + strlen(metric_str) + strlen(yyvsp[-17]) + 9) > MAXLEN  ) {
           ...
           graph_data_global.all_trace_steps.add_step( trace_step_key,
	   			  rulenum, metric_str, fact1, factQ);

           // save unique rule
           graph_data_global.ruleList.add_rule(rulenum, desc_str);
...
#line 201 "graphit.y"
        {
                ...
                printf("attack(%s).\n\n",fact1_str);
                #endif
		graph_data_global.goals[fact1_str] = NULL;
                fact1_str[0] = 0;
                lastFact=0;
		fact1=0;
...
#line 295 "graphit.y"
              { 
                   ...
                   if( strlen(fact1_str) ==  0 ) {
                    ...
                    Predicate *p = 
                     graph_data_global.all_predicates.add_predicate( yyvsp[-3], arg_count, undef);

                    // add this fact to the fact list, unless 
                    // it is already in the list. 
                    lastFact=graph_data_global.all_facts.add_fact( fact_ptr,p, arglist_p);
